...
public sealed partial class HomeAssistantWebSocketClient : IDisposable
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            await ConnectAsync(cancellationToken).ConfigureAwait(false);
            if (registration.ServerId is null
                || !_serverSubscriptions.TryGetValue(registration.ServerId.Value, out var activeRegistration)
    ...
    {
        _subscriptions.TryRemove(registration.Id, out _);
        var serverId = registration.ServerId;
        if (serverId is null)
        {
            return;
        }
